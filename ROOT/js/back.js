...
$(document).on("click",".news-more", function() {
	...
	for(var i=0;i<newsdata.length;i++){
		if(itemid==newsdata[i].id){
			...
			console.log(newsdata[i].picture);
		}
	}
...
$(document).on("click", ".ser-more", function() {
	...
	for(var i=0;i<serdata.length;i++){
		if(itemid==serdata[i].id){
			html+='<h4>'+serdata[i].name+'</h4><div class="middle"><h4>'+serdata[i].tid+'</h4></div><img class="newsimg" src="'+XHR.SERVICES_URL+serdata[i].picture+'" alt=" "/><p>'+serdata[i].introduction+'</p><div class="video"><video src="'+serdata[i].video+'" controls="controls"></video></div><div class="item-set"><a class="edititem editseritem">更新</a><a class="list menuitem menuseritm">回目录</a><a class="delitem delseritem">删除</a></div>';	
			console.log("2"+serdata[i].picture);
			//console.log(serdata[i].picture)
		}
	}
...
$(document).on("click",'.fileSubmit',function(){
 	...
 	var data = new FormData(parent); 
 	if (ptag == 'Pic'){
 		sibling1 = $(this).siblings('#image');
 		data.append('image',sibling1[0].files[0]);
 	} else if(ptag == "Vi"){
 		sibling2 = $(this).siblings('#video');
 		data.append('video',sibling2[0].files[0]);
 	}
 	var status=$(this).siblings('.status');
 	var preview=$(this).siblings('.preview');
 	var vis=$(this).siblings('.vis');
 	var pics=$(this).siblings('.pics');
 	

 	status.html('正在上传...');
	$.ajax({ 
		url: XHR.getUrl({func:'upload'+ptag}), 
		type: 'POST', 
		data: data, 
		dataType: 'JSON',
		cache: false, 
		...
		success: function(res){
			if(res.code){
				...
				if(ptag=="Vi"){
					...
					preview.html('<video src="'+XHR.SERVICES_URL+res.data+'" controls="controls"></video>');
				}else{
					pics.html(res.data);
					
					preview.html('<img src="'+XHR.SERVICES_URL+res.data+'" />');
...
/*$('.tvideo').html('123456');
console.log($('.tvideo').html())
$('.tpicture').html('123456');
console.log($('.tpicture').html())*/